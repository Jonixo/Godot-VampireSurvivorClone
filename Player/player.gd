...
func get_random_item():
	...
	for i in UpgradeDb.UPGRADES:
		if i in collected_upgrades: ##already have
			pass
		elif i in upgrade_options: #already an option
			pass
		elif UpgradeDb.UPGRADES[i]["type"] == "item": #dont pick food
			pass
		elif UpgradeDb.UPGRADES[i]["prerequisite"].size() > 0: #check prereqs
			var to_add = true
...
func death():
	...
	tween.tween_property(deathPanel,"position",Vector2(220,50),1.0).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	tween.play()
	if time >= 300:
